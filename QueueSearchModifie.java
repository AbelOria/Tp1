 ...
 */
public abstract class QueueSearchModifie extends NodeExpander {
	...
	private int rechercheCurent = 0;
	
	private int rechercheMaximum = Integer.MAX_VALUE;  
	
	public void setRechecheMaximum(int rm){
		rechercheMaximum = rm;
	}
	
	public int getNoeudsExplores(){
		return rechercheCurent;
	}
...
//		while (!(frontier.isEmpty()) && !CancelableThread.currIsCanceled()) {
		while (!(frontier.isEmpty()) && !CancelableThread.currIsCanceled()&& rechercheCurent < rechercheMaximum) {
